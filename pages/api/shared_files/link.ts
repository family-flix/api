...
export default async function handler(
  ...
  const { url, file_id, file_name, target_file_id } = req.body as Partial<{
    ...
    file_name: string;
    /** 网盘文件夹 file_id。如果指定了该参数，就不会使用 file_name 去网盘中找同名，直接使用该参数 */
  ...
  const r1 = await store.find_shared_files_in_progress({
    url,
    file_id,
    name: file_name,
    user_id,
  ...
  if (!shared_file_in_progress) {
    const r2 = await store.add_shared_files_in_progress({
      url,
      file_id,
      name: file_name,
      user_id,
  ...
  if (target_file_id) {
    ...
    // 删除同名的、失效的关联关系
    store.delete_shared_files_in_progress({
      file_id,
      user_id,
      need_update: 1,
    });
