...
async function run() {
  await walk_table_with_pagination<EpisodeRecord & RecordCommonPart>(
    store.find_episodes_with_pagination,
    {
      async on_handle(episode) {
        ...
        const tv_res = await store.find_tv({ id: tv_id });
        if (tv_res.error) {
        ...
        const existing_res = await store.find_folder({ file_id });
        if (existing_res.error) {
          console.log(
            "fetch existing folder failed",
            existing_res.error.message
          );
          return;
        }
        if (existing_res.data) {
          if (!existing_res.data.size && size) {
            ...
            await store.update_folder(existing_res.data.id, { size });
          }
        ...
        // 补全缺少的 folder 记录
        const r = await store.add_folder({
          file_id,
          name: file_name,
          type: 1,
          parent_file_id,
