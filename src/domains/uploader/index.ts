 ...
 * 2、将二进制文件保存至本地
 */
import { rename, ReadStream, createWriteStream, writeFileSync, unlink, writeFile } from "fs";
...
export class FileManage {
  ...
  upload_subtitle_v2(file: File, filename: string): Promise<Result<string>> {
    return new Promise(async (resolve) => {
      const subtitle_path = path.resolve(this.root, "subtitle", filename);
      const buffer = new Uint8Array(await file.arrayBuffer());
      writeFile(subtitle_path, buffer, (err) => {
        if (err) {
          return resolve(Result.Err(err.message));
        }
        return resolve(Result.Ok("/" + path.relative(path.resolve(this.root), subtitle_path)));
