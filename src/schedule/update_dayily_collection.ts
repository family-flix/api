...
async function updateUserDailyUpdate(user: User, store: DatabaseStore) {
...
export async function main(values: { store: DatabaseStore; user?: User }) {
  const { store, user } = values;

  if (user) {
    await updateUserDailyUpdate(user, store);
    return;
  }
