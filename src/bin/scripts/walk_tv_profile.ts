...
async function walk_tv_profile(store: DatabaseStore) {
  ...
  await walk_records(store.prisma.tv_profile, {}, async (profile) => {
    const { id, tmdb_id, name, original_name, poster_path, backdrop_path, created } = profile;
    const body: Partial<{
      poster_path: string;
    ...
    if (p.poster_path && p.poster_path.includes("http")) {
      const r = await upload.download(p.poster_path, `/poster/${tmdb_id}.jpg`);
      if (r.data) {
        body.poster_path = r.data;
      }
    }
    if (p.backdrop_path && p.backdrop_path.includes("http")) {
      const r = await upload.download(p.backdrop_path, `/backdrop/${tmdb_id}.jpg`);
      if (r.data) {
