...
export default async function v0_history_list(req: NextApiRequest, res: NextApiResponse<BaseApiResp<unknown>>) {
  const e = response_error_factory(res);
  const { authorization } = req.headers;
  const { page: page_str, page_size: page_size_str } = req.body as Partial<{
    episode_id: string;
    page: string;
    page_size: string;
  }>;
