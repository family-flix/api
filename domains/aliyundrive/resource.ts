...
export class AliyunResourceClient extends BaseDomain<TheTypesOfEvents> {
  static async Get(options: Partial<{ drive_id: string; store: DatabaseStore }>) {
    ...
    // console.log('[DOMAIN]aliyun/resource - before drive', drive.drive_token);
    ...
    // console.log("[DOMAIN]aliyundrive/resource - Get", token_res.data);
    return Result.Ok(
  ...
  async init() {
    const token_res = await (async () => {
      ...
      if (!expired_at || dayjs(expired_at * 1000).isBefore(dayjs())) {
        ...
        const refresh_token_res = await this.refresh_access_token();
  ...
  async fetch_video_preview_info(file_id: string) {
    const r1 = await this.ensure_initialized();
    if (r1.error) {
    ...
    }>(API_V2_HOST + "/v2/file/get_video_preview_play_info", {
      file_id,
      drive_id: this.drive_id,
