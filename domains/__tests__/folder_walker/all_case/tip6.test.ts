...
describe("detect a tv dir", () => {
  afterEach(async () => {
    await store.operation.clear_dataset("episode");
    await store.operation.clear_dataset("season");
    await store.operation.clear_dataset("tv");
    await store.operation.clear_dataset("folder");
  });
  test("tip6", async () => {
