...
import { ModelQuery, DriveRecord, FileRecord } from "@/domains/store/types";
import { folder_client, walk_model_with_cursor } from "@/domains/store/utils";
...
export class Drive extends BaseDomain<TheTypesOfEvents> {
  ...
  async delete_folder(f: { file_id: string; name: string; drive_id: string }) {
    ...
    const folder = new Folder(file_id, {
      name,
      client: folder_client({ drive_id }, this.store),
    });
   ...
   * 删除云盘内的指定文件
   */
  async delete_file_or_folder_in_drive(file_id: string) {
